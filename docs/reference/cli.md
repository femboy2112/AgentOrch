...
| `--watchdog-scale` | `FLOAT` | `None` | Multiply the streaming-watchdog stall/byte budgets (>1.0) for known-heavy tiers so a long `xhigh` run isn't truncated mid-flight. Env `AGY_WATCHDOG_SCALE`. |

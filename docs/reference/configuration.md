...
| `AGY_WATCHDOG_SCALE` | `harness/roles.py:212` | Multiplier applied to the calibrated `max_bytes` and `stall` budget (loosen/tighten the watchdog). | `1.0` (no scaling) | Float `>0`; empty/`0`/non-numeric → no scaling. |

...
class _MasterStubAgent(AgentInstance):
    ...
    async def run_async(self, piped_input: Optional[str] = None) -> str:
        if "Lead Architect" in self.prompt:
            return json.dumps([_LONG_STEP, "Step two"])
...
class _FallbackUsageFail(AgentInstance):
    @classmethod
    async def get_available_models(cls):
        return ["stub"]
    ...
    async def run_async(self, piped_input: Optional[str] = None) -> str:
        self.stderr = "429 rate limit exceeded"
        raise RuntimeError("simulated usage wall")


class _VerifierPass:
    async def verify(self, working_directory: str) -> VerifierResult:
        return VerifierResult(ok=True, message="ok")


class _SeqStaticAgent(_StaticAgent):
    def __init__(self, *args, replies: List[str], **kwargs):
        super().__init__(*args, **kwargs)
        self._replies = list(replies)

    async def run_async(self, piped_input: Optional[str] = None) -> str:
        if self._replies:
            self.reply = self._replies.pop(0)
        return await super().run_async(piped_input)


_LONG_STEP = "Step one " + ("x" * 160)
...
def test_master_emits_plan_and_step_transitions():
    ...
    assert plan_rows[0]["step_titles"] == [_LONG_STEP[:120], "Step two"]
    ...
    assert step_rows[0]["step_title"] == _LONG_STEP[:120]
    assert step_rows[0]["model"] == "m"
    assert step_rows[0]["effort"] == "low"
    assert step_rows[1]["step_title"] == _LONG_STEP[:120]
    assert step_rows[2]["step_title"] == "Step two"
    assert step_rows[2]["model"] == "m"
    assert step_rows[2]["effort"] == "low"
    assert step_rows[3]["step_title"] == "Step two"
...
def test_tree_of_thought_emits_selected_branch():
    ...
    assert selected[0]["branch_total"] == 2
    assert selected[0]["selector"] == "judge"
    assert selected[0]["score"] == 9
    assert selected[0]["scores"] == [2, 9]


def test_tree_of_thought_vote_emits_scores_and_selector():
    events: List[dict] = []
    branches = [
        _StaticAgent(prompt="x", reply="same"),
        _StaticAgent(prompt="x", reply="same"),
        _StaticAgent(prompt="x", reply="other"),
    ]
    tot = TreeOfThought(
        branch_instances=branches,
    ...
    selected = [r for r in rows if r.get("phase") == "tot" and r.get("action") == "branch_selected"]
    assert len(selected) == 1
    assert selected[0]["selected_branch"] == 1
    assert selected[0]["branch_total"] == 3
    assert selected[0]["selector"] == "vote"
    assert selected[0]["score"] == 2
    assert selected[0]["scores"] == [2, 2, 1]


def test_adversarial_emits_iteration_start_and_completion():
    events: List[dict] = []
    wf = AdversarialReview(
        generator_instance=_StaticAgent(prompt="", reply="draft", model="gen-model", effort="medium"),
...
def test_adversarial_emits_verified_outcome():
    events: List[dict] = []
    wf = AdversarialReview(
        generator_instance=_StaticAgent(prompt="", reply="draft", model="gen", effort="low"),
        critic_instance=_StaticAgent(prompt="", reply="ignored"),
        verifier=_VerifierPass(),
        max_iterations=2,
        event_callback=events.append,
    )
...
def test_adversarial_emits_continue_outcome():
    events: List[dict] = []
    wf = AdversarialReview(
        generator_instance=_StaticAgent(prompt="", reply="draft"),
        critic_instance=_SeqStaticAgent(prompt="", replies=["needs changes", "APPROVED"]),
        max_iterations=3,
        event_callback=events.append,
    )
    ...
    assert [(r["action"], r["iteration"], r["iteration_total"]) for r in rows] == [
        ("iteration_started", 1, 3),
        ("iteration_completed", 1, 3),
        ("iteration_started", 2, 3),
        ("iteration_completed", 2, 3),
    ]
    assert rows[1]["outcome"] == "continue"
    assert rows[3]["outcome"] == "approved"
...
def test_fallback_emits_reroute_transition_on_rollover():
    ...
    assert rows[0]["reason_category"] == "error"
    assert rows[0]["attempt"] == 1
    assert rows[0]["attempt_total"] == 2


def test_fallback_emits_usage_reason_category():
    events: List[dict] = []
    Agent = make_fallback_agent([_FallbackUsageFail, _FallbackOk], cycles=1)
    agent = Agent(prompt="task")
    agent.event_callback = events.append
    ...
    rows = [r for r in _orchestration_rows(events) if r.get("phase") == "fallback"]
    assert len(rows) >= 1
    assert rows[0]["reason_category"] == "usage"
    assert rows[0]["attempt"] == 1
    assert rows[0]["attempt_total"] == 2


def test_fieldless_orchestration_event_validates():
    events: List[dict] = []
    wf = AdversarialReview(
        generator_instance=_StaticAgent(prompt="", reply="draft"),
        critic_instance=_StaticAgent(prompt="", reply="APPROVED"),
    ...
    wf._emit_orchestration()
    rows = _orchestration_rows(events)
    assert rows == [{"workflow": "master"}]
